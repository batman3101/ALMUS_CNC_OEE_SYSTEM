 ...
 *
 * ## 이벤트만으로는 부족했다 — 그래서 상태도 읽게 한다
 *
 * 구독은 "만료되는 **순간**"만 전달한다. 그런데 문제가 생기는 자리는 그 순간이 아니라
 * 그 **직후**다. 만료 시점에 이미 날아가 있던 요청들의 `catch` 는 조금 늦게 깨어나
 * `message.error('대시보드 데이터를 불러오는데 실패했습니다')` 를 띄운다. 그 `catch` 는
 * 이벤트를 놓쳤으므로 물어볼 곳이 없었고, 그래서 데이터 탓을 했다.
 *
 * `isSessionExpired()` 는 그 `catch` 가 물어볼 수 있는 **사실**이다. 순서는 보장된다 —
 * `authFetch` 는 응답을 돌려주기 **전에** `notifySessionExpired()` 를 동기적으로 부르므로,
 * 401 때문에 실행되는 모든 `catch` 는 이미 참이 된 이 값을 본다.
 */
 ...
 * "지금 세션이 끝난 상태인가."
 *
 * 중복 알림 차단과 같은 값을 쓴다 — 둘은 같은 사실의 두 쓰임새이고, 따로 두면 반드시
 * 어긋난다. 로그인을 다시 시도하는 순간(`resetSessionExpiryNotice`) 거짓으로 돌아간다.
 */
 ...
 * 세션이 끝난 상태인지 묻는다.
 *
 * 실패를 도메인 언어로 옮겨 적기 전에 이걸 본다 — 참이면 원인은 그 도메인이 아니라
 * 로그인이고, 화면에는 이미 만료 안내가 떠 있다. `@/lib/errorReporting` 참고.
 */
