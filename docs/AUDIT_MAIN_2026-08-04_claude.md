...
### 7. Next.js 16.2.10 — 보안 패치 이전 버전 ✅ 해소(2026-08-04)

`package.json` `"next": "^16.2.10"`, 실제 설치본도 `16.2.10`. 권고 GHSA-6gpp-xcg3-4w24 의
영향 범위는 `>=16.0.0 <16.2.11`, 패치는 `16.2.11`.

**16.2.11 로 올렸고, `next` 자체 권고는 사라졌다**(`npm audit` 재실행으로 확인).

> **정정** — 이 절은 처음에 `npm audit` 의 "High 3, Moderate 3" 을 Next.js 문제인 것처럼
> 적었다. 실제로는 **하나도 Next.js 자체 권고가 아니었다.** 근원은 전이 의존성이다:
>
> | 등급 | 패키지 | 영향 범위 | 경로 |
> |---|---|---|---|
> | HIGH | `sharp` | `<0.35.0` | `next` 가 고정 |
> | HIGH | `brace-expansion` | `<1.1.18`, `>=4.0.0 <5.0.8` | 빌드·린트 도구 |
> | MODERATE | `postcss` | `<=8.5.22` | `next` 가 고정 |
> | MODERATE | `tar` | `<=7.5.20` | 빌드 도구 |
>
> `npm audit` 이 `next` 를 HIGH 로 표시하는 것은 **next 가 취약한 sharp/postcss 에
> 의존하기 때문**이지 next 에 결함이 있어서가 아니다. 패키지 이름만 보고 등급을 옮겨
> 적으면 이런 오독이 생긴다.

**남은 6건에 대한 방향** — `npm audit fix`(비강제)로는 풀리지 않는다(dry-run 확인).
`sharp` 는 `next@16.2.11` 이 고정하므로 Next 가 올려 주기 전에는 `overrides` 강제 외에
방법이 없고, 그건 이미지 최적화를 깨뜨릴 위험이 있어 이 작업에서는 하지 않았다.

노출 면은 좁다 — **앱 코드는 `sharp` 를 직접 쓰지 않는다.** 이미지 업로드
(`/api/upload/image`)는 MIME·크기만 검증하고 Supabase Storage 로 바로 넘기며 어떤 이미지
처리도 하지 않는다. `sharp` 는 Next 의 이미지 최적화 경로에서만 돌고, `next.config` 에
`images` 설정이 없다. 그래도 Next 가 sharp 를 올리면 함께 올릴 항목으로 남긴다.
