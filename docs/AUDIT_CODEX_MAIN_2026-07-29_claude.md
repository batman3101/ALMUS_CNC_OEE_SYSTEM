...
> 이 문서는 감사·계획 문서다. **코드 수정도 마이그레이션 적용도 하지 않았다.**
> 운영 반영은 별도 승인 후에 진행한다.
...
- `git log -S "youkillme"` → 최소 `acde36f`, `d3849cb` 두 커밋에 남아 있다. 현재 파일만 고쳐도 **히스토리에서 사라지지 않는다.**
...
`src/**` 전체를 훑어 `password:` / `passwd` 다음에 문자열 리터럴이 오는 패턴, 그리고 `@gmail.com` 등 실제 이메일 리터럴을 금지한다. 주석 안도 검사 대상에 포함한다 — 이번 건이 정확히 주석 안에서 났다.
