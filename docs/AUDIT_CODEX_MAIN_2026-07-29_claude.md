...
> 이 문서는 감사 결과와 수정 방안이다. 아래 "수정 적용 현황"이 이 브랜치에서 실제로
> 무엇을 고쳤고 무엇을 남겼는지 말한다. **마이그레이션은 파일만 작성했고 운영 DB 에는
> 적용하지 않았다.** Edge Function 도 소스만 고쳤고 배포하지 않았다.

---

## 수정 적용 현황 (브랜치 `audit/claude-2026-07-29-codex-main-audit`)
...
| 3 | RLS 우회 | — | ⏸ 마이그레이션 2건 **작성만** (미적용) | `rlsPolicyLedger.test.ts` |
| 4 | Edge Fn 인가 | ✅ 소스 수정 | ⏸ **배포 안 함** | `dailyOeeAggregationAuthz.test.ts` |
| 5 | 임의 교대 진척 | ✅ 라우트 시간창 가드 | ⏸ 마이그레이션 1건 작성만 (미적용) | `shiftReportingWindow.test.ts` + 라우트 테스트 6건 |
| 6 | 마감 비원자 | ⏭ **미착수** (아래 사유) | — | — |
| 7 | 설정 실패 위장 | ✅ fail-loud + 중복 사본 2개 제거 | — | `settingsFailLoud.test.ts` |
| 8 | Realtime 유실 | ✅ 이벤트 버퍼 + 스냅샷 후 재생 | — | `realtimeBuffer.test.ts` |
| 9 | 종료시각 무시 | ✅ UI 파생 표시로 전환 (ko/vi) | — | (타입·빌드) |
| 10 | anon grant | — | ⏸ 마이그레이션 1건 작성만 (미적용) | — |

**#6 을 남긴 이유**: 발생 빈도가 가장 낮고(마감은 교대 종료 후에만 실행되므로 창이 사후
정정에만 열린다) 수정 비용이 가장 크다 — 새 RPC `close_shift_upsert_v2` 를 만들고 코드 배포
후 구 함수를 지우는 **2단계 배포**가 필요하다. 나머지가 운영에 반영된 뒤 별도로 진행한다.

**검증** — ESLint 0 error / 18 warning(기준선과 동일), TypeScript 통과, Jest **103 suites
674 tests 전부 통과**(기준선 96/614 대비 +7 suites, +60 tests), 프로덕션 빌드 성공,
마이그레이션 드리프트 0 · 미적용 4(의도).

**운영 반영 순서** — 11절의 브랜치별 순서를 따르되, 각 마이그레이션 파일 끝에 적힌
"적용 전 확인"을 먼저 통과시킨다. 특히 `20260729140000`(읽기 스코프화)은 **EXPLAIN ANALYZE
전후 비교와 세 역할 수동 확인**이 선행되어야 한다.
