...
~~정정 버튼은 마이그레이션이 미적용이므로 500 또는 RPC 없음 오류가 정상이다.~~

**갱신 (2026-07-29): 마이그레이션 적용 완료.** 정정은 실제로 성공해야 한다. 브라우저 확인
시 다음 세 경로를 구분할 것:

| 상황 | 기대 |
|---|---|
| 열린 `downtime_entries` 가 있는 설비 | 성공. 시작 시각 유지, 로그 분할 없음 |
| `machine_logs` 로만 내려간 설비 | **성공** (20260729010000 이전에는 500 이었다) |
| 이미 가동 재개된 설비 | 409 → "이미 가동 중이라 정정할 수 없습니다" |
...
1. ~~**`resolveDowntimeReasonLabel` 의 폴백**~~ → **해결 (2026-07-28): 원문 코드 노출로 확정.**
   실측상 실제 데이터 13종이 전부 두 사전 중 하나에 있어 폴백 도달은 0건이다. 이 자리에
   도달하는 경우는 사실상 번역 누락뿐이므로, 화면에 코드가 그대로 뜨는 편이 원인을 즉시
   특정하게 해 준다. "기타"로 접지 않은 이유는 `other` 가 이미 실제 사유 코드로 91건
   존재해 진짜 기타와 섞이면 분석에서 되짚을 수 없기 때문이다.
2. ~~**마이그레이션 적용**~~ → **완료 (2026-07-28):** `20260728115346` + `20260728115539`
   (audit_log.action 이 varchar(20) 인데 값이 23자여서 RPC 가 항상 실패하던 버그 수정판).
   롤백 트랜잭션에서 "일반 상태 전환은 여전히 로그를 남긴다" 회귀까지 확인. 원장은
   `applied` + `hashes` 로 이동 완료.
3. **main 병합** — 아직 하지 않는다. 사용자가 명시적으로 지시할 때만. PR 도 아직 만들지 않는다.
