...
인증·권한·보안 항목은 범위에서 제외했다. 최초 발견은 코드 흐름을 근거로 한 정적 감사였고, 수정 후에는 회귀 테스트와 실제 브라우저 읽기 경로를 검증했다. 운영 DB 쓰기는 신규 migration을 먼저 적용해야 하므로 자동 수행하지 않았다.
...
- 16개 항목의 코드 원인을 모두 제거하고 12개 Jest 스위트, 61개 회귀 테스트로 경계 조건을 고정했다.
- 3개월 전체 설비 실제 데이터에서 OEE 원본 `142,822`건, 생산실적 `326,971`건을 확인했다.
- 원본 목록은 5,000건 단위 안정 정렬 페이지를 사용하며 첫 2페이지 10,000건에서 중복 ID 0건을 확인했다.
- 생산실적과 수동 비가동은 신규 `save_daily_production_with_downtime` RPC에서 한 트랜잭션으로 저장한다.
- 브라우저 검증 기록은 `docs/qa/bug-fix-2026-07-15/BROWSER_VERIFICATION.md`를 참조한다.
- 신규 DB 함수·트리거는 `supabase/migrations/20260715120000_atomic_downtime_save.sql` 적용 후 실제 저장 smoke test가 필요하다.
...
- 기존 비가동이 있는 교대를 휴무로 전환할 때 빈 배열로 원자 삭제
- 생산실적 또는 어느 한 교대의 비가동 조회 실패 시 전체 저장 차단
- A/B 교대 범위 밖 비가동 거부와 B교대 자정 교차 허용
