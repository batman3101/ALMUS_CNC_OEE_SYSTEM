...
-- 아니며, 경계는 그대로 제약이 지킨다.
--
-- ## audit_log 는 여기서 다루지 않는다
--
-- `audit_log(table_name, record_id, ...)` 는 다형 참조라 부모를 정적으로 알 수 없다.
-- 유일한 쓰기 경로인 `audit_role_change` 는 **역할 변경**을 기록하는데, 역할은 공장별
-- membership 이 아니라 `user_profiles` 의 전역 역할이다. 그 감사 기록이 어느 공장에
-- 속하는지는 운영 결정이 필요하므로, 지금은 비워 두고 별도로 정한다.
-- (그 함수가 도는 경로는 사용자 역할 변경뿐이고, 이번 다중화 범위 밖이다.)
